...
@section('container')
<div class="m-4">
  <a href="/admin/siswa/create" class="btn mb-4">Tambah Peserta Didik Baru</a>
  ...
  <div class="div-table">
    <table class="table w-full">
      ...
      <tbody>
            ...
            <div class="flex items-center space-x-3">
              <div class="avatar">
                <div class="mask mask-squircle w-12 h-12">
                  @if ($siswa->image)
                    <img src="{{ asset('storage/'.$siswa->image) }}" alt="" class="w-full"/>
                  @else
              ...
              <div>
                <div class="font-bold">{{ ucwords($siswa->nama) }}</div>
                <div class="text-sm opacity-50">NISN : {{ $siswa->nisn }}</div>
              </div>
            </div>
          </td>
          <td>
            {{ ucwords($siswa->tempatlahir) }}, {{ date('d-m-Y', strtotime($siswa->tanggallahir) )}}
            <div class="text-sm opacity-50">Usia : {{ $siswa->usia }} Tahun</div>
          </td>
          <td>{{ Str::limit(ucwords($siswa->alamat), 30) }}</td>
          <td><a href="/admin/siswa?status={{ $siswa->status }}">{{ $siswa->status }}</a></td>
          <td>
            <div>
              <div class="tooltip tooltip-info" data-tip="Lihat">
                <a href="/admin/siswa/{{ $siswa->slug }}" class="btn btn-ghost btn-sm btn-square"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg></a>
              </div>
        ...
        @endforeach
        <tr>
          <td colspan="5">{{ $pesertadidik->links('pagination::tailwind') }}</td>
        </tr>
      </tbody>
    </table>
  </div>
